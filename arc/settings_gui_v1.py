...
class settings():
    ...
    def __init__(self):
        ...
        #makes a function that destroys the gui and writes all the settings to the settigns.json file
        def back():
        ...
        #this makes a function that changes the color of the button when they are pressed 
        def clrchg(numb):
        ...
        #this function is used to update the settings when each button is pressed
        def press(numb):
            if settings_list[numb] == True:
        ...
        #these are debug settings that i added to make testing easier
        debug_settings_list = [
            ["Delete calc_cache on exit", 1, lambda:press(3), 0,1],
            ["Delete exports on exit", 1,lambda:press(4),1,1]
        ]
        #this is what is used to add the debug settings to the regular settings
        #it only does that if the debug variable is true
        if debug == True:
            button_detail_list.extend(debug_settings_list)
       
        #list to hold the buttons once they are made
        self.button_ref_list = []
        #this function makes all the button using the information in the button_detail_list
        ...
        #didn't get round to making this and would be a bit of a pain to remove so here it will stay
        self.sf_button = self.button_ref_list[0].config(state=DISABLED)
        #this is what opens the stored settings so they are persistent across instances 
        try:
            with open("settings.json") as j:
                ...
                numb = 0
                #this sets the correct color for the settings if they aren't the default
